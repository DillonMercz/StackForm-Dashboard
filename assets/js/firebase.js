...
const firebaseConfig = {
    ...
    authDomain: "stackform-ae310.firebaseapp.com",
    projectId: "stackform-ae310",
    storageBucket: "stackform-ae310.appspot.com",
...
function createUser() {
    ...
    firebase.auth().createUserWithEmailAndPassword(email, password)
        .then((userCredential) => {
            ...
            fetch("https://stackform.obimedia.agency/onboard", requestOptions)
        ...
        if (user) {
            ...
            db.collection("users").doc(uid).collection("forms").doc(id).set({
                ...
                .then(() => {
                    ...
                    fetch("https://stackform.obimedia.agency/create/" + id + "/" + userEmail, requestOptions)
                        .then(response => response.text())
    ...
    firebase.auth().onAuthStateChanged((user) => {
        if (user) {
            ...
            db.collection("users").doc(uid).collection("forms").doc(doc).delete().then(() => {
                ...
                fetch("https://stackform.obimedia.agency/delete/" + doc, requestOptions)
                    .then(response => response.text())
...
function createTable(id) {
    ...
    globalId = id
    var json
    fetch("https://stackform.obimedia.agency/leads/" + id + '-leads.js')
        .then(response => response.text())
        .then(result => {
            if (result.charAt(0) == ",") {
                                    ...
                                    <div class="my-1 px-1 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3">
                                        ...
                                        <article class="overflow-hidden rounded-lg shadow-lg">
                                            ...
                                            <header class="flex items-center justify-between leading-tight p-2 md:p-4">
                                                <h1 class="text-lg">
                                                    <a class="no-underline hover:underline text-black" href="./pages/team.html?team=${doc.id}">
                                                        ${doc.data().name}
                                                    </a>
                                                </h1>
...
function assignToTeam(team) {
    ...
    fetch("https://stackform.obimedia.agency/teams/submit/" + team, requestOptions)
        .then(response => response.text())
...
function createTeamTable(id) {
    ...
    var json
    fetch("https://stackform.obimedia.agency/teams/" + id + '-leads.js')
        .then(response => response.text())
        .then(result => {
            if (result.charAt(0) == ",") {
                result = result.slice(1);
...
function deleteLead(form, lead) {
    ...
    fetch("https://stackform.obimedia.agency/leads/delete/" + form + "/" + lead, requestOptions)
        .then(response => response.text())
        ...
        if (user) {
            ...
            db.collection("users").doc(uid).collection("teams").doc(document1).get().then((doc) => {
                if (doc.exists) {
                    ...
                    fetch("https://stackform.obimedia.agency/alert/members", requestOptions)
                        .then(response => response.text())
                        .then(result => window.location.reload())
                        .catch(error => console.log('error', error));
        ...
        if (user) {
            ...
            db.collection("users").doc(uid).collection("prospects").get()
                .then((querySnapshot) => {
                    ...
                    console.log(Object.keys(querySnapshot.docs).length)
                    if(Object.keys(querySnapshot.docs).length == 0){
                      document.getElementById('leads').innerHTML = `<center><h3>Download Our Chrome Extension</h3></center><br><center><button style="width: 180px;border: none;background: #52b36c;color: white;" onclick="window.open('https://firebasestorage.googleapis.com/v0/b/stackform-ae310.appspot.com/o/Stackform%20Prospect%202.zip?alt=media&token=b6b53296-5a64-4b61-9d67-c37c11593c49')">Download Now</button>`
                      document.getElementById('headers').innerHTML = ""
                    }else{
                    var template = ""
                    querySnapshot.forEach((doc) => {
        ...
        if (user) {
            ...
            db.collection("users").doc(uid).collection("prospects").get()
                .then((querySnapshot) => {
                    ...
                    var fileTitle = "Stackform-Prospects"; // or 'my-unique-title'
